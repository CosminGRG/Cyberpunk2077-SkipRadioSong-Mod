...
function input.onInit()
    ...
    ObserveBefore("PlayerPuppet", "OnGameAttached", function()
        ...
        Game.GetCallbackSystem():RegisterCallback("Input/Key", input.inputListener:Target(),
            input.inputListener:Function("OnKeyInput"), true)
    end)
...
---@return table
function input.createBindingInfo(nativeSettingsPath, keybindLabel, isHoldLabel, keybindDescription, isHoldDescription, id,
                                 maxKeys, maxKeysLabel, maxKeysDescription, supportsHold, defaultOptions, savedOptions,
...
local function addKeybind(info, index, nativeSettings) -- Add single keybind widget
    ...
    if info.supportsHold then
        input.nuiTables[holdID] = nativeSettings.addSwitch(info.nativeSettingsPath, info.isHoldLabel .. numberText,
            info.isHoldDescription, info.savedOptions[holdID], info.defaultOptions[holdID], function(state)
                input.nuiTables[numID].isHold =
    ...
    input.nuiTables[numID] = nativeSettings.addKeyBinding(info.nativeSettingsPath, info.keybindLabel .. numberText,
        info.keybindDescription, info.savedOptions[numID], info.defaultOptions[numID], info.savedOptions[holdID],
        function(key)
...
function input.addNativeSettingsBinding(info) -- Add combined hotkey widget from info table
    ...
    if not nativeSettings.pathExists(info.nativeSettingsPath) then
    ...
    if info.maxKeys ~= 1 then -- Add slider to change amount of key widgets
        nativeSettings.addRangeInt(info.nativeSettingsPath, info.maxKeysLabel, info.maxKeysDescription, 1, info.maxKeys,
            1, info.savedOptions[maxID], info.defaultOptions[maxID], function(value)
